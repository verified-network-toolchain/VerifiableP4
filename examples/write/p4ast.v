...
Definition myHeader_t := DeclHeader tt {| stags := tt; str := "myHeader_t" |}
    [(MkDeclarationField tt (TypBit 8) {| stags := tt; str := "firstByte" |});
     (MkDeclarationField tt (TypBit 8)
          {| stags := tt; str := "secondByte" |})].
...
Definition headers := DeclStruct tt {| stags := tt; str := "headers" |}
    [(MkDeclarationField tt
          (TypHeader
           [(MkFieldType {| stags := tt; str := "firstByte" |} (TypBit 8));
            (MkFieldType {| stags := tt; str := "secondByte" |} (TypBit 8))])
    ...
    [(MkParserState tt {| stags := tt; str := "start" |}
          [(MkStatement tt
                (StatMethodCall
                     ...
                     [(TypHeader
                       [(MkFieldType {| stags := tt; str := "firstByte" |}
                             (TypBit 8));
                        (MkFieldType {| stags := tt; str := "secondByte" |}
                             (TypBit 8))])]
                     [(Some
                       (MkExpression tt
                            (ExpExpressionMember
          ...
          [(MkTableKey tt
                (MkExpression tt
                     (ExpExpressionMember
                          ...
                          {| stags := tt; str := "firstByte" |}) (TypBit 8)
          ...
          (Some
           (MkTableActionRef tt
                (MkTablePreActionRef
                     (BareName {| stags := tt; str := "drop" |}) nil)
                TypVoid))
          (Some {| itags := tt; value := 256; width_signed := None |}) nil)]
    (BlockCons
         (MkStatement tt
              (StatConditional
                   (MkExpression tt
...
Definition program := [decl1; packet_in; packet_out; verify; NoAction; decl2;
                       ...
                       HashAlgorithm; mark_to_drop; mark_to_drop; hash;
