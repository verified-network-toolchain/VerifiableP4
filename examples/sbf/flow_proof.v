...
Definition Packet:Type := (Time * Header) * Payload.
...
| fw_outgoing: forall f t h data, is_internal (SrcAddr h) = true ->
               fw_process_packet f ((t,h),data) (insert f (t, h)) (Some (h, data))

(*Incoming packet is permitted by the firewall*)
| fw_incomingPermit: forall f t h data, is_internal (SrcAddr h) = false ->
               In (query (clear f t) (t, Build_Header (DstAddr h) (SrcAddr h)))
                  [None; Some true] ->
               fw_process_packet f ((t,h),data) (clear f t) (Some (h, data))

(*this is the case where packets are dropped, ie DOS is detected*)
| fw_incomingDrop: forall f t h data, is_internal (SrcAddr h) = false -> 
               In (query (clear f t)  (t, Build_Header (DstAddr h) (SrcAddr h)))
                  [ None; Some false] ->
               fw_process_packet f ((t,h),data) (clear f t) None.
...
Definition get_tstamp (p : Packet) : Time := (fst (fst p)).

Definition get_src (p : Packet) : ip_addr := SrcAddr (snd (fst p)).

Definition get_dst (p : Packet) : ip_addr := DstAddr (snd (fst p)).
...
Proof. induction 1; unfold get_tstamp; simpl; intros.
...
+ assert (X: (hh, ht) :: h <> []) by (clear; intros N; list_solve).
  ...
  - apply IHtrans_fw; clear IHtrans_fw. clear H4; subst t0.
     ...
     * list_solve.
     * unfold Packet in *. 
       replace ((@Zlength (Time * Header * Payload) h + 1
                - @Zlength (Time * Header * Payload)  ((hh, ht) :: h)))
       with 0 by list_solve.
       replace (@Zlength (Time * Header * Payload)
            ((hh, ht) :: h) - 1) 
       with (@Zlength (Time * Header * Payload) h) by list_solve.
...
+ assert (Ha: exists u r1, fw_process_packet s a u r1).
  { clear IHf. destruct a as [[ta h] data].
    ...
    - remember (query (clear s ta) (ta, Build_Header (DstAddr h) (SrcAddr h))) as q; destruct q.
      * destruct b.
        { specialize (fw_incomingPermit s ta h data Heqdir).
          rewrite <- Heqq. intros X. do 2 eexists.
          apply X. right; left; trivial. }
        { specialize (fw_incomingDrop s ta h data Heqdir).
          rewrite <- Heqq. intros X. do 2 eexists.
          apply X. right; left; trivial. }
      * specialize (fw_incomingDrop s ta h data Heqdir).
          rewrite <- Heqq. intros X. do 2 eexists.
...
Definition property (h : list Packet) (p : Packet) (res : Result) : Prop :=
    is_internal (get_src p) = false /\ is_internal (get_dst p) = true /\
...
Lemma flow_level_property : forall (h : list Packet) (p : Packet) (res : Result),
  ...
  valid_flow (h ++ [p]) ->
  property h p res.
Proof. intros. red. intros [Hsrc [Hdst [i [Hi [iRecent [iSrc iDst]]]]]].
destruct H as [result [f [g [Trans Step]]]].
destruct p as [[tp header] data].

inv Step; simpl; trivial.
(*Interesting case: DROP*)
unfold get_src, get_dst in *. simpl in *. clear H6. 
...
[ 
|rewrite H in H7; destruct H7 as [Q | [ Q | Q]]; congruence; contradiction].
clear H7.
...
assert (exists state1 res1 state2 resI res2, 
  ...
  result = res1++resI::res2 /\ state2 = insert state1 (fst (Znth i h))).
{ clear- Hh Trans iDst iSrc Hdst Hsrc. (*symmetry in Hh.*) rewrite app_assoc in Hh.
...
assert (Hstate2: state2 = insert state1 (fst (Znth i h))).
...
assert (ValidTailTp: valid_flow (tail++[(tp, ((sH, dH) : Header), data)])).
...
assert (QI: query (insert state1 (fst (Znth i h)))
   (tp, ((dH, sH) : Header)) = Some true).
{ subst sH dH. destruct (Znth i h). simpl in *.
  destruct p; simpl in *. destruct h0. simpl in *. apply query_insert_same.
  + simpl in *.
...
+ destruct (@trans_cons_inv tail (a :: tail) _ _ _ Tail _ (eq_refl _))
  ...
  { clear - ValidTailTp. generalize dependent a. induction tail; simpl; intros.
    ...
    ** unfold Header in *.
       replace (a0 :: a :: tail ++ [(tp, headerI, data)]) with ([a0] ++ ( a :: tail ++ [(tp, headerI, data)])) in ValidTailTp by reflexivity.
       destruct (valid_flow_app_inv ValidTailTp).
       apply IHtail in H0; clear IHtail.
       replace ([a0] ++ a :: tail ++ [(tp, headerI, data)]) 
           with (([a0;a] ++ (tail ++ [(tp, headerI, data)]))) in ValidTailTp by reflexivity.
       destruct (valid_flow_app_inv ValidTailTp).
