...
Definition digest_t := DeclStruct NoInfo
    {| stags := NoInfo; str := "digest_t" |}
    [(MkDeclarationField NoInfo
          (TypTypeName {| stags := NoInfo; str := "ipv4_addr_t" |})
          {| stags := NoInfo; str := "src_addr" |});
     (MkDeclarationField NoInfo
...
Definition SwitchIngressParser := DeclParser NoInfo
    ...
    [(DeclInstantiation NoInfo
     ...
     (DeclInstantiation NoInfo
          (TypSpecializedType
               (TypTypeName
                {| stags := NoInfo; str := "EtherIPTCPUDPParser" |}) nil) nil
          {| stags := NoInfo; str := "layer4_parser" |} nil)]
    [(MkParserState NoInfo {| stags := NoInfo; str := "start" |}
          [(MkStatement NoInfo
                (StatMethodCall
                     (MkExpression NoInfo
          ...
          [(MkExpression NoInfo
           ...
           (MkExpression NoInfo
                (ExpInt
                 {| itags := NoInfo; value := 0;
                    width_signed := (Some ( 32%N, false )) |}) (TypBit 32%N)
                Directionless)] {| stags := NoInfo; str := "reg_counter" |}
          nil);
     (DeclInstantiation NoInfo
          (TypSpecializedType
          ...
          {| stags := NoInfo; str := "regact_counter" |}
          [(DeclFunction NoInfo TypVoid {| stags := NoInfo; str := "apply" |}
                ...
                (BlockCons
                     ...
                     (BlockCons
                          ...
                          (BlockEmpty NoInfo))))]);
     (DeclAction NoInfo {| stags := NoInfo; str := "act_counter" |} nil nil
          (BlockCons
               (MkStatement NoInfo
                    (StatAssignment
                         (MkExpression NoInfo
                         ...
                         (MkExpression NoInfo
                              ...
                              (TypBit 32%N) Directionless)) StmUnit)
               (BlockEmpty NoInfo)));
     (DeclTable NoInfo {| stags := NoInfo; str := "tbl_counter" |} nil
          [(MkTableActionRef NoInfo
                (MkTablePreActionRef
                             ...
                             (MkExpression NoInfo
                                  (ExpName
                                   (BareName
                                    {| stags := NoInfo;
                                       str := "tbl_counter" |})
                                   (LInstance ["tbl_counter"]))
                                  (TypTable
                                   {| stags := NoInfo;
                                      str := "apply_result_tbl_counter" |})
                                  Directionless)
                             {| stags := NoInfo; str := "apply" |})
                        (TypFunction
                         (MkFunctionType nil nil FunTable
    ...
    [(MkParameter false Directionless
     ...
     (MkParameter false In
          (TypTypeName
           {| stags := NoInfo;
              str := "ingress_intrinsic_metadata_for_deparser_t" |}) 
          None {| stags := NoInfo; str := "ig_intr_dprsr_md" |})] nil
    [(DeclInstantiation NoInfo
          (TypSpecializedType
               (TypTypeName {| stags := NoInfo; str := "Digest" |})
               [(TypTypeName {| stags := NoInfo; str := "digest_t" |})]) nil
          {| stags := NoInfo; str := "digest" |} nil)]
    (BlockCons
         (MkStatement NoInfo
              (StatConditional
                   (MkExpression NoInfo
                        (ExpBinaryOp Eq
                             ...
                             (MkExpression NoInfo
                                  ...
                                  (TypBit 3%N) Directionless)) TypBool
                        Directionless)
                   (MkStatement NoInfo
                        (StatBlock
                         (BlockCons
            ...
            ( {| stags := NoInfo; str := "udp" |},
              (TypHeader
               [( {| stags := NoInfo; str := "src_port" |}, (TypBit 16%N) );
                ...
                ( {| stags := NoInfo; str := "checksum" |}, (TypBit 16%N) )]) )]);
          (TypStruct
           [( {| stags := NoInfo; str := "num_pkt" |}, (TypBit 32%N) )]);
          (TypStruct
           [( {| stags := NoInfo; str := "ethernet" |},
              (TypHeader
               [( {| stags := NoInfo; str := "dst_addr" |}, (TypBit 48%N) );
...
Definition prog := Program
    [decl'1; packet_in; packet_out; verify'check'toSignal; NoAction; decl'2;
     ...
     EmptyEgressDeparser; EmptyEgress; digest_t; metadata_t;
     EtherIPTCPUDPParser; SwitchIngressParser; SwitchIngress;
