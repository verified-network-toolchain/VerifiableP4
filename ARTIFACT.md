...
- 182 (packet-level process relation) [VerifiableP4/examples/sbf/switch.v](examples/sbf/switch.v)
...
## Admitted Lemmas
There is one admitted lemma `exec_expr_det_empty_det` in [VerifiableP4/core/Hoare.v](core/Hoare.v) which is unprovable. But its only usage is in evaluating table entries where expressions should satisfy `exec_expr_det_empty_det`. So it is not a big concern. A refactoring that will bypass and remove this admitted lemma is undergoing.
